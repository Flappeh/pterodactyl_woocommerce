...
def download_file(server_full_uuid:str,download_url:str,client_header:str):
    ...
    result = requests.post(url=url_download,headers=client_header,json=body)



def verify_user(user_id:int,server_id:int,requested_type:int,app_header:str):
    url_verify_ownership=f'https://panel.infinity-projects.de/api/application/servers/{server_id}'
    result = requests.get(url=url_verify_ownership,headers=app_header).json()
